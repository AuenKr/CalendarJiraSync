...
export async function logTimeForDateInActiveCalendarTab({
  date,
  lastLoggedTime,
}: {
  date: string
  lastLoggedTime?: string | null
}): Promise<LogTimeRunResult> {
  const configured = await isJiraConfigured()
  if (!configured) {
    return {
      loggedCount: 0,
  ...
  if (!tab.url?.includes('calendar.google.com')) {
    return {
      ...
      errors: 0,
      eligibleCount: 0,
      message: 'Please go to Google Calendar',
      isError: true,
    }
  }

  try {
    return await runLogTimeForDate({
      date,
      lastLoggedTime,
      fetchEvents: async () => {
        const response = await chrome.tabs.sendMessage(tab.id!, { type: 'FETCH_CALENDAR_EVENTS' })
        return (response?.events || []) as CalendarEvent[]
      },
      fetchDescription: async (eventId: string) => {
        const response = await chrome.tabs.sendMessage(tab.id!, {
          type: 'FETCH_EVENT_DESCRIPTION',
          payload: { eventId },
        })
        return response?.description as string | undefined
      },
    })
  } catch (e: unknown) {
    const err = e as Error
    const message = err.message?.includes('Receiving end does not exist')
      ? 'Please refresh the Calendar page'
      : 'Failed. Refresh Calendar page.'
