...
const MOUNT_POINT_ID = 'jira-sync-extension-root'

console.log('[Jira Sync] Content script loaded')

function injectApp(modal: Element) {
  console.log('[Jira Sync] Attempting to inject into modal', modal)

  if (modal.querySelector(`#${MOUNT_POINT_ID}`)) {
    console.log('[Jira Sync] Already injected')
    return
  }
  ...
  if (titleInput && titleInput.parentElement) {
    console.log('[Jira Sync] Found title input', titleInput)
    ...
    if (target && target.parentElement) {
      console.log('[Jira Sync] Injecting after target', target)
      target.insertAdjacentElement('afterend', host)
      injected = true
    } else if (titleInput.parentElement) {
      console.log('[Jira Sync] Fallback: Injecting after title input parent')
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message.type === 'FETCH_CALENDAR_EVENTS') {
    console.log('[Jira Sync] Received request to scrape events')
    const events = scrapeEvents()
    console.log('[Jira Sync] Scraped events:', events)
    sendResponse({ events })
  ...
  if (!injected) {
    console.log('[Jira Sync] Title input not found or injection failed. Trying generic append.')
    ...
    console.log('[Jira Sync] Appending to content', content)
  ...
  try {
    ...
    console.log('[Jira Sync] React app mounted')
  } catch (e) {
    console.error('[Jira Sync] Failed to mount React app', e)
  }
}
    ...
    for (const node of mutation.addedNodes) {
      if (node instanceof Element) {
        ...
        if (node.getAttribute('role') === 'dialog') {
          console.log('[Jira Sync] Detected new dialog', node)
          injectApp(node)
        } else {
          const dialog = node.querySelector('[role="dialog"]')
          if (dialog) {
            console.log('[Jira Sync] Detected dialog inside added node', dialog)
            injectApp(dialog)
          }
          ...
          if (node.classList.contains('yDmH0d') || node.querySelector('.yDmH0d')) {
             console.log('[Jira Sync] Detected event bubble', node)
             injectApp(node)
          }
          
          // Check for full page edit container
          if (node.querySelector('#xTiIn') || node.querySelector('.p9lUpf')) {
             console.log('[Jira Sync] Detected full edit page', node)
             injectApp(node)
          }
        }
      }
...
if (existingDialog) {
  ...
  if (fullEdit) {
    console.log('[Jira Sync] Found existing full edit page')
    injectApp(fullEdit)
  }
}
